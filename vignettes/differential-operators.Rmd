...
The function [`gradient`](https://calculus.guidotti.dev/reference/gradient.html) implements the symbolic and numeric gradient of `functions`, `expressions` and `characters`. In Cartesian coordinates:
...
Numerical methods are applied when working with `functions` with the same sintax introduced for [derivatives](https://calculus.guidotti.dev/reference/derivative.html).
...
The function [`jacobian`](https://calculus.guidotti.dev/reference/jacobian.html) is a wrapper for [`gradient`](https://calculus.guidotti.dev/reference/gradient.html) that always returns the Jacobian as a matrix, even in the case of unidimensional scalar-valued functions.
...
When the function $F$ is a tensor-valued function $F_{d_1,\dots,d_n}$, the [`hessian`](https://calculus.guidotti.dev/reference/hessian.html) is computed for each scalar component.
...
where $J=\prod_ih_i$. When $F$ is an `array` of vector-valued functions $F_{d_1,\dots,d_n,i}$, the [`divergence`](https://calculus.guidotti.dev/reference/divergence.html) is computed for each vector:
...
where $J=\prod_i h_i$. This suggests to implement the [`curl`](https://calculus.guidotti.dev/reference/curl.html) in $m+2$ dimensions in such a way that the formula reduces correctly to the previous cases:

$$(\nabla \times F)_{k_1\dots k_m} = \frac{h_{k_1}\cdots h_{k_m}}{J}\sum_{ij}\epsilon_{ijk_1\dots k_m}\partial_i\Bigl(h_jF_j\Bigl)$$

And in particular, when $F$ is an `array` of vector-valued functions $F_{d_1,\dots,d_n,i}$ the [`curl`](https://calculus.guidotti.dev/reference/curl.html) is computed for each vector:
...
where $J=\prod_ih_i$. When the function $F$ is a tensor-valued function $F_{d_1,\dots,d_n}$, the [`laplacian`](https://calculus.guidotti.dev/reference/laplacian.html) is computed for each scalar component:
